...
async function run() {
    try {
        ...
        if (output.length > 0) {
            totalPercentage = (0, utils_1.calculateOverallScore)(output);
            if (rott_token !== '') {
                const [organization, repository] = github_1.context.repo.repo.split('/');
                const branchName = github_1.context.ref.split('/').pop() || '';
